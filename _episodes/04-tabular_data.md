...
## Plotting Data
Another common way to analyze tabular data is to graph it.  To graph our data, we will need a new python library that contains functions to plot data.  
...
{: .language-python}
```
fig_1 = matplotlib.pyplot.plot(data[:,0])
```
{: .language-python}
(Need to figure out how to include the graph here.)

We should label our axes.
```
matplotlib.pyplot.xlabel('Simluation Frame')
matplotlib.pyplot.ylabel('Distance (angstroms)')
fig_1 = matplotlib.pyplot.plot(data[:,0])
```
Sometimes when people import python modules they give them a shorthand name so that they have to type less.  For example,
```
import matplotlib.pyplot as pyp
pyp.xlabel('Simluation Frame')
pyp.ylabel('Distance (angstroms)')
fig_1 = pyp.plot(data[:,0])
```
